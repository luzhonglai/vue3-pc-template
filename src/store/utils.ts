...
type ModuleNameType = 'test1' | 'permission' | 'app' | 'authAdmin' | 'loginGlobal'
...
export function setStoreState<T>(module: ModuleNameType, key: keyof T, value: unknown) {
  store.commit({
    type: module + '/__set',
    key: key,
    val: value
 ...
 */
export function dispatchAction<T>(module: ModuleNameType, key: keyof T, value?: unknown, async?: boolean) {
  if (async) {
